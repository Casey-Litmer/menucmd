...
def batch() -> str | None:
    ...
    out = sp.run(["pyinstaller", script_path, "--name", exe_name,
                  "--distpath", outpath, "--specpath", outpath, "--workpath", outpath + r".\build", "--onefile"],
...
def change_relative_path_behaviour(python_script_path: str, exe_dir: str) -> str:
    ...
    temp_script_path = "_" + os.path.basename(python_script_path)

    with open(temp_script_path, "w") as temp_script:
        temp_script.write(monkey_patch + "".join(lines))
    temp_script.close()

    return os.path.abspath(temp_script_path)
