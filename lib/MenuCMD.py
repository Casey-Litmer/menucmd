...
#WIP#
class Item():
    def __new__(cls, key: str, message: str, *funcs_arg_pairs: tuple) -> tuple:
        XYZ = ()
        for p in map(tupler, funcs_arg_pairs):
            XYZ += (Bind(p[0], p[1:]),)
        return (key, message, XYZ)
...
class Menu():
    ...
    #Kwargs Wrapper
    class kwargs():
        """Wrapper for in-line keyword arguments.
        Accepts a dict or direct **kwargs.  (**kwargs will override dict keys)"""
        class kwargs(dict):
            pass
        def __new__(cls, kwargs_dict = {}, **kwargs):
            return cls.kwargs(kwargs_dict | kwargs)
    ...
    def __call__(self):
        ...
        while len(switch) >= 2:
            ...
            kwargs = dict_union(tupler(x for x in args if isinstance(x, Menu.kwargs.kwargs)))
            args = tupler(x for x in args if not isinstance(x, Menu.kwargs.kwargs))
